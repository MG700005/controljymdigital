  ...
  for (let i = startIndex; i < rows.length; i += 1) {
    ...
    const record = {
      ...
      previewEnviado: parseBoolean(
        hasHeader ? row[indexMap.preview] : hasEmprendimiento ? row[2] : row[1]
      ),
      periodoCaducado: parseBoolean(
        hasHeader ? row[indexMap.caducado] : hasEmprendimiento ? row[3] : row[2]
      ),
      pagoEfectuado: parseBoolean(
        hasHeader ? row[indexMap.pago] : hasEmprendimiento ? row[4] : row[3]
      ),
      createdAt: Number(
        hasHeader ? row[indexMap.createdAt] : hasEmprendimiento ? row[5] : row[4]
      ) || Date.now(),
      notes: sanitizeNotes(
        hasHeader ? row[indexMap.notes] || "" : hasEmprendimiento ? row[6] || "" : row[5] || ""
      ),
    };
