...
function loadRecords() {
    try {
        ...
        return parsed
            .filter((record) => record && record.whatsapp !== undefined && record.whatsapp !== null)
            .map((record) => ({
                ...record,
...
function getVisibleRecords() {
    ...
    const filtered = state.records.filter((record) => {
        const whatsappText = record.whatsapp ? record.whatsapp.toLowerCase() : "";
        const matchesSearch =
            !search ||
            whatsappText.includes(search) ||
...
function renderPortfolio() {
    ...
    const lines = items.map(([name, link]) => {
        const parts = [name];
        if (link) {
            parts.push(link);
        }
    ...
    portfolioText.value = lines.join("\n\n");
}
...
copyPortfolio.addEventListener("click", async () => {
    if (!portfolioText.value.trim()) {
        setFormError("No hay portafolio para copiar.");
        return;
    }

    try {
        await navigator.clipboard.writeText(portfolioText.value);
        setFormSuccess("Portafolio copiado.");
    } catch (error) {
        portfolioText.select();
